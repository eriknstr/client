...
func checkAndRecordBodyHash(g *libkb.GlobalContext, bodyHash chat1.Hash, uniqueMsgID chat1.MessageID, uniqueConvID chat1.ConversationID) error {
	...
	if err != nil {
		// TODO: Suppress IO errors here and just log them?
		return err
	}
	if found {
	...
	return g.LocalChatDb.PutRaw(bodyHashKey, bodyHashValue)
}

func ChatDbBackedBodyHashChecker(g *libkb.GlobalContext) BodyHashChecker {
	return func(bodyHash chat1.Hash, uniqueMsgID chat1.MessageID, uniqueConvID chat1.ConversationID) error {
		return checkAndRecordBodyHash(g, bodyHash, uniqueMsgID, uniqueConvID)
	}
}
...
func checkAndRecordPrevPointer(g *libkb.GlobalContext, msgID chat1.MessageID, convID chat1.ConversationID, uniqueHeaderHash chat1.Hash) error {
	...
	return g.LocalChatDb.PutRaw(prevKey, headerHashVal)
}

func ChatDbBackedPrevChecker(g *libkb.GlobalContext) PrevChecker {
	return func(msgID chat1.MessageID, convID chat1.ConversationID, uniqueHeaderHash chat1.Hash) error {
		return checkAndRecordPrevPointer(g, msgID, convID, uniqueHeaderHash)
	}
}
